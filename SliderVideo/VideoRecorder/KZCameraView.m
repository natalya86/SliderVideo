    ...
    if (self) {
        if ([self captureManager] == nil) {
            ...
            if ([[self captureManager] setupSession]) {
                ...
                CGRect frame = CGRectMake(134, 2, 80, 80);
                self.radialView = [[MDRadialProgressView alloc]initWithFrame:frame];
                self.radialView.progressTotal = 80;
                ...
                self.radialView.center = CGPointMake(self.frame.size.width/2, self.videoPreviewView.frame.size.height + (screen.height-384)/2);
                ...
                [self.radialView addGestureRecognizer:longPress];
...
//                [self setFocusModeLabel:newFocusModeLabel];
                ...
                self.deleteLastBtn.center = CGPointMake(self.videoPreviewView.frame.size.height-44, self.videoPreviewView.frame.size.height -44);
//                
...
- (void) updateDuration
{
    if ([[[self captureManager] recorder] isRecording])
    {
        ...
        self.radialView.progressCounter = self.duration*10;
        
        
        [self.nextitem setEnabled:YES];
        if (self.duration > 80) {
            [self.durationTimer invalidate];
